...
$queryMaxMinDate="SELECT max(CreationDate), min(CreationDate) FROM `EbayTransactions`";
	...
	function getTotalSaleForMonth($startDate,$endDate)
{
   ...
   */
   ...
   $query="SELECT SKU,Title,SUM(QTY) AS output,SellingPrice,SUM(SellingPrice*Qty) as subtotal,ItemId,Shipping,CostPrice,SUM(CostPrice*Qty) FROM EbayTransactions Where SKU!='' AND SellingPrice > 0.00 AND CreationDate <='".$endDate."'and CreationDate >='".$startDate."' GROUP BY Title ORDER BY `output` DESC";
