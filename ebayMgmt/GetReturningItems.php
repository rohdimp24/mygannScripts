...
if(isset($_GET['fromYear'])&& isset($_GET['toYear']))
{
    ...
    $queryItem = "SELECT group_concat(DISTINCT(year(CreationDate))),EbayTransactions.SKU,group_concat(qty),group_concat(SellingPrice),EbayTransactions.Title,group_concat(ItemId), EbayProductCost.CostPrice
                FROM `EbayTransactions`,`EbayProductCost` where EbayTransactions.Title=EbayProductCost.Title and year(CreationDate)>='" . $startYear . "' and year(CreationDate)<='" . $endYear . "' and SellingPrice > '0'and EbayTransactions.SKU!='' group by EbayTransactions.SKU";
    ...
    for ($i = 0; $i < $rownum; $i++) {
        ...
        if($showRepeat) {
            ...
            if (sizeof($years) == $totalYears) //then the custome ris present for both 2014 and 2015
            {
				...
				#get the list of the unique ebayIds
				$arrEbayIds=explode(",",$row[5]);
				$arrEbayIdsUnique=array_unique($arrEbayIds);
				$ebayIds=implode(",",$arrEbayIdsUnique);
    ...
    $lines .= "<tr><th>Sno</th><th>SKU</th><th>Product Name</th><th>Total Item Bought</th><th>Total Spent</th><th>CP per unit</th><th>Years Bought</th></tr>";
    $count = 1;
    for ($i = 0; $i < sizeof($arrItems); $i++) {

        $lines .= "<tr><td>" . $count++ . "</td><td>" . $arrItems[$i]->sku ."</td><td>".$arrItems[$i]->productName."</td><td>". $arrItems[$i]->totalQty . "</td><td>" . $arrItems[$i]->totalSale . "</td><td>".$arrItems[$i]->costPrice."</td><td>".$arrItems[$i]->years."</td></tr>";
    }
    $lines .= "</table></div>";
