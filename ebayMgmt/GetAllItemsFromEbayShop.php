...
function getProductsFromStore($totalPages)
{
    ...
    */
    ...
    //$arrCategory=array();
    $updateString='';
    //$totalPages=2; //for now limit this number
...
function updateEbayDBWithFile_old($fileName)
{
              ...
              //  $retna.="<tr>".$item->title."</tr>";
                ...
                if(strlen($sku)>1)
                {
                    ...
                    */
                    ...
                    if($rowsnumCheck>0)
                        //$query="UPDATE `EbayProductsForTx` SET `Title`= '".mysql_real_escape_string($title)."',`SKU`='".$sku."',`StartDate`='".$startDate."', `EndDate`='".$endDate."',`continueDiscontinue`='".$isDiscontinued."' where EbayItemId='".$itemId."'";
                        ...
                        fullPhoto='".$fullPhoto."',itemUrl='".$itemUrl."',sellingPrice='".$sellingPrice."' where EbayItemId='".$itemId."'";

                    else
                    {
                        //get the size of the image also
                        if(strlen($thumbnail)>10)
                        {
                        ...
                        $query="INSERT INTO `EbayProductsForTx`(`EbayItemId`, `Title`, `SKU`, `SellingStatus`, `StartDate`, `EndDate`, 
                            `continueDiscontinue`,`thumbnail`,`fullPhoto`,`itemUrl`,`sellingPrice`,`thumbnailWidth`,`thumbnailHeight`)
                                VALUES ('".$itemId."','".mysql_real_escape_string($title)."','".$sku."','".$sellingStatus."',
                                    '".$startDate."','".$endDate."','".$isDiscontinued."','".$thumbnail."','".$fullPhoto."',
                                    '".$itemUrl."','".$sellingPrice."','".$width."','".$height."')";
...
$updateString=getProductsFromStore($totalPages);


echo $updateString;
//these are the products that got updated..that is they are still valid products. The products that were not updated from the product listing 
//are the onew which are obsolete now and need to be not consdered
...
//compare the updated products with all the existing products ..the one which dont match in the updated array needs to be discontinued
