...
if(isset($_POST["txtItemId"])&&isset($_POST["productSubmit"])){
    ...
    $queryProducts="SELECT Title, ItemId,Month(SellingDate),YEAR(SellingDate),sum(Qty),
    sellingPrice FROM `AmazonTransactions` WHERE `ItemId` like '".$itemId."' 
     group by YEAR(SellingDate), MONTH(SellingDate), sellingPrice order by Year(SellingDate) Desc,ItemId, sellingPrice";
    ...
    for($i=0;$i<$rowsnumProducts;$i++)
    {
        ...
        if($oldId!=$internalKey)
        {
            ...
            $queryExpenses="SELECT `ItemId`, `CostPrice`, `Shipping` FROM `AmazonProductCost` 
            where ItemId='".$itemFetched."'";   
            ...
            if(isset($obj))
            {
                $obj->amazonFee=floatval($obj->totalSale*0.20);
				...
				{
					$obj->profit=$obj->totalSale-($obj->totalQty*($obj->costPrice+$obj->shipping)+$obj->amazonFee);
					$denom=$obj->totalQty*($obj->costPrice+$obj->shipping)+$obj->amazonFee;
    ...
    for($i=0;$i<sizeof($arrData);$i++)
    {
        ...
        if($currentYear!=$obj->year)
        {
            ...
            if($i>0)
			{
				$totalDenom=($totalCP+$totalShipping+$totalAmazonFee);
				if($totalDenom<0.1)
                ...
                <td>'.number_format($totalAmazonFee,2,'.',',').'</td>
